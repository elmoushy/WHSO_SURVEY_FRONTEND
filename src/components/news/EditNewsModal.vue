...
import { updateSliderNews, updateNormalNews, uploadSliderImage, uploadNormalImage } from '../../services/newsService'
...
const modalTitle = computed(() => 
  props.newsItem?.news_type === 'SLIDER' ? 'Edit Slider News' : 'Edit Normal News'
)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Update news item
    const updateFn = props.newsItem.news_type === 'SLIDER' ? updateSliderNews : updateNormalNews
    ...
    if (imageFile.value) {
      const uploadFn = props.newsItem.news_type === 'SLIDER' ? uploadSliderImage : uploadNormalImage
    ...
    error.value = err.response?.data?.detail || 'Failed to update news. Please try again.'
  } finally {
...
<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show && newsItem" :class="$style.overlay" @click.self="handleClose">
        <div :class="$style.modal">
          ...
          <div :class="$style.content">
            ...
            <form @submit.prevent="handleSubmit">
              ...
              <div v-if="newsItem.main_image" :class="$style.formGroup">
                ...
                <div :class="$style.currentImage">
                  ...
                  <span :class="$style.imageInfo">
                    {{ newsItem.main_image.original_filename }} 
                    ({{ Math.round(newsItem.main_image.file_size / 1024) }}KB)
                  </span>
                </div>
              </div>
              ...
              <div :class="$style.formGroup">
                <label :class="$style.label">
                  {{ newsItem.main_image ? 'Replace Image (Optional)' : 'Upload Image (Optional)' }}
                </label>
                ...
                <div v-else :class="$style.uploadArea">
                  ...
                  <label for="imageUploadEdit" :class="$style.uploadLabel">
                    ...
                    <span :class="$style.uploadText">
                      {{ newsItem.main_image ? 'Click to upload new image' : 'Click to upload image' }}
                    </span>
